...
const Hero = () => {
    ...
    return (
        ...
        <section className="relative overflow-hidden bg-[#121212] px-6 py-24">
            <div className=" absolute left-1/2 top-0  h-125 w-125 -translate-x-1/2 rounded-full bg-blue-500/10 blur-3xl"/>
            <div className=" relative mx-auto grid max-w-7xl items-center gap-20 lg:grid-cols-2">
                <div>
                    <div className=" mb-6 inline-flex items-center gap-2  rounded-full border border-neutral-700 bg-neutral-900 px-4 py-2 text-sm text-neutral-300 ">
                        <Sparkles size={16} className="text-yellow-400"/>
                        AI Powered Study Assistant
                    </div>
                    <h1 className=" text-5xl font-bold leading-tight text-white lg:text-6xl ">
                        Study Smarter with
                        <span className="text-blue-400">{" "}Neural Docx</span>
                    </h1>
                    ...
                    <div className=" mt-10 flex flex-wrap gap-4" >
                        {
                            isAuthenticated ? 
                            (
                                <Link to="/chat" className=" inline-flex items-center  gap-2 rounded-xl bg-white px-6 py-3 font-semibold  text-black transition hover:bg-neutral-200 ">
                                    Open Chat
                                    <ArrowRight size={18} />
                                </Link>
                            ) : 
                            (
                                <Link to="/register" className=" inline-flex items-center gap-2 rounded-xl bg-white px-6 py-3 font-semibold text-black  transition hover:bg-neutral-200">
                                    Get Started
                                    <ArrowRight size={18} />
                                </Link>
                            )
                    ...
                    <div className="grid gap-5">
                        {
                            features.map((feature) => (
                                <div key={feature} className="flex gap-4">
                                    <CheckCircle2 size={22} className=" mt-1 shrink-0 text-green-400 "/>
